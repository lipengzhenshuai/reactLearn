...
import { editContainer, addContainer, generatepolyphonePop, polyphoneSelect, generatePreview } from "./content.js";
import { getNode, getFontSiezList, getFontFamilyList, getFontColorList, isPromise, getWordNameByValue } from "./utils/utils";
import { getPinYin } from "./utils/generate";
import { createContainer, insertAfter, getUpELement, removeContainer, otherDomUpdate } from "./utils/dom.js";
...
export default function generate({data = defaultConfig, onSubmit = () => {}, 
	...
	createContainer();
	// 设置下拉框模板
	getSelectList = getSelectList(pyFontFamilys, wordFontFamilys, pyFontSizes, wordFontSizes, wordColors, pyColors);
	// 1.初始化基本数据
...
const wordOptions = config => {
	...
	<div id="${IDs.FOURLINE}" data-value="2" class="py-list-item disable  ${wordType === 2 ? 'active': ''}">
		<span class="py-low2">${four_line}</span>
		<span>四线三格</span>
	</div>
	<div id="${IDs.SQUARE}" data-value="3" class="py-list-item disable  ${wordType === 3 ? 'active': ''}">
		<span class="py-low2">${square}</span>
		<span>田字格</span>
	</div>
	<div id="${IDs.COMBINE}" data-value="4" class="py-list-item disable  ${wordType === 4 ? 'active': ''}">
		<span>${combine}</span>
		<span>组合形式</span>
	</div>`;
};
...
const pinyinOptions = config => {
	...
	<div id="${IDs.FIRSTUP}" data-value="1" class="py-list-item ${pinyinType === 1 ? 'active': ''} disable">
		<span class="py-low2">${first_upper_font}</span>
		<span>首字大写</span>
	</div>
	<div id="${IDs.ALLUP}" data-value="2" class="py-list-item ${pinyinType === 2 ? 'active': ''} disable last">
...
const panel2 = config => {
	...
	const { options: {wordStyle: {fontFamily, fontSize, color} } } = config;
	const { nabla, font } = decorationSvgs;
...
let getSelectList = (pyFontFamilys, wordFontFamilys, pyFontSizes, wordFontSizes, wordColors, pyColors) => {
...
const addEventAgent = () => {
	...
	editContent.addEventListener("click" , e => {
		...
		const innerClickElement = getUpELement(e.target, "popOut_pys", "py-edit-content");

...
export const insertAfter = (newElement, targetElement) => {
    ...
    if (parent.lastChild == targetElement) {
		...
		for(let i = newElement.length; i > 0;  i--) {
			parent.appendChild(newElement[i - 1]);
		}
    } else {
		for(let i = newElement.length; i > 0;  i--) {
...
export const getUpELement = (currentEle, targetClassOrTagname, endEleClassOrTagment) => {
	...
	else return getUpELement(currentEle.parentNode, targetClassOrTagname, endEleClassOrTagment)
}
